...
const Utils = {
  ...
  showToast: (message, type = 'success') => {
    ...
    }[type] || { bg: '#dcfce7', border: '#16a34a', text: '#15803d', icon: '✓' };
    ...
    el.style.cssText = `position:fixed;bottom:24px;right:24px;z-index:9999;
      ...
      box-shadow:0 4px 20px rgba(0,0,0,0.12);max-width:300px;
      transform:translateY(16px);opacity:0;transition:all 0.28s ease;`;
    el.innerHTML = `<span style="width:22px;height:22px;border-radius:50%;background:${c.border};
      color:white;display:flex;align-items:center;justify-content:center;font-size:12px;flex-shrink:0;">${c.icon}</span>
      <span>${message}</span>`;
    document.body.appendChild(el);
    requestAnimationFrame(() => { el.style.transform = 'translateY(0)'; el.style.opacity = '1'; });
    setTimeout(() => { el.style.transform = 'translateY(16px)'; el.style.opacity = '0';
      setTimeout(() => el.remove(), 280); }, 2800);
  },

  showNotification: (msg) => Utils.showToast(msg, 'success'),
  confirm: (msg) => window.confirm(msg),

  getStatusBadge: (status) => {
    const map = { Pending:'badge-warning', Confirmed:'badge-info', Preparing:'badge-info',
...
// ============================================================
// NAV INIT — updates user icon + keeps cart as link to cart.html
// ============================================================
const NavInit = {
  init: () => {
    const user = Auth.getCurrentUser();
    ...
    if (user) {
      ...
      userLink.innerHTML = `
        <div style="position:relative;display:flex;align-items:center;gap:6px;">
          ...
          <span style="font-size:13px;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
            ${name.split(' ')[0]}</span>
          <span style="position:absolute;top:-3px;right:-3px;width:8px;height:8px;
            background:#16a34a;border-radius:50%;border:1.5px solid white;"></span>
        </div>`;
      ...
      userLink.addEventListener('click', (e) => {
        ...
        dd.id = 'user-dd';
        dd.style.cssText = `position:fixed;top:64px;right:16px;z-index:9000;
...
const PageInit = {
  ...
  index: async () => {
    ...
    try {
      ...
      grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#888;padding:40px;">Could not load products. Make sure the server is running.</p>';
    }
    setupProductButtons();
  },
      ...
      try {
        ...
        catFilter.innerHTML = ['All', ...cats].map(cat => `
          <button class="category-btn ${cat === selectedCategory ? 'active' : ''}"
            onclick="selectCategory('${cat}')">${cat}</button>`).join('');
    ...
    const renderProducts = async () => {
      grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 0;color:#aaa;">
        <div style="width:32px;height:32px;border:3px solid #dc2626;border-top-color:transparent;
          border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 12px;"></div>Loading...</div>`;
      try {
        const products = await api.getProducts(selectedCategory === 'All' ? null : selectedCategory);
        if (!products.length) {
          grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#888;padding:60px;">No products in this category.</p>';
          return;
        }
        grid.innerHTML = products.map(p => productCardHTML(p)).join('');
        setupProductButtons();
      } catch {
        grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#888;padding:60px;">Failed to load. Make sure the server is running.</p>';
      }
    };
  ...
  cart: () => {
        ...
        itemsEl.innerHTML = items.map(item => `
          <div class="cart-item" id="cart-item-${item.id}">
            <img src="${item.image || 'https://via.placeholder.com/96'}"
              class="cart-item-image" onerror="this.src='https://via.placeholder.com/96'" />
            <div class="cart-item-details" style="flex:1;">
              <h3 class="cart-item-title">${item.name}</h3>
              <p class="cart-item-category">${item.category || ''}</p>
              <div class="quantity-controls">
                <button class="quantity-btn" onclick="cartUpdateQty(${item.id}, -1)">
            ...
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:12px;">
              ...
              <button onclick="cartRemove(${item.id})" style="
                ...
                onmouseout="this.style.background='#fee2e2'">Remove</button>
            </div>
          </div>`).join('');
      }
    ...
    window.cartRemove = (id) => {
      Cart.removeItem(id);
      renderCart();
      if (Cart.getItems().length === 0) Utils.showToast('Cart is now empty', 'info');
    };
    ...
    document.getElementById('checkout-btn')?.addEventListener('click', () => {
      ...
      // Normal cart checkout — no buy-now flag
  ...
  checkout: () => {
    ...
    // Show Buy Now notice if applicable
    if (buyNow) {
      ...
      if (notice) {
        ...
        notice.innerHTML = `<div class="alert alert-info" style="margin-bottom:16px;">
          <strong>Buy Now:</strong> Ordering "${buyNow.name}" directly. 
          <a href="cart.html" style="color:#1d4ed8;">Go to cart</a> to order multiple items.</div>`;
      }
    }
  ...
  confirmation: () => {
    ...
    if (!order) {
      document.querySelector('main').innerHTML = `<div style="text-align:center;padding:80px 20px;">
        ...
        <a href="menu.html" class="btn btn-primary">Go to Menu</a></div>`;
      return;
    }
    ...
    if (itemsEl && order.items) {
      itemsEl.innerHTML = order.items.map(i => `
        <div style="display:flex;justify-content:space-between;padding:8px 0;
          ...
          <span style="color:#374151;">${i.name} <span style="color:#888;">x${i.quantity}</span></span>
  ...
  orders: async () => {
    ...
    try {
      ...
      container.innerHTML = orders.map(order => {
        ...
        return `<div class="card" style="margin-bottom:20px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#dc2626,#f97316);
            ...
            <div>
              <p style="color:rgba(255,255,255,.8);font-size:12px;margin-bottom:2px;">Order Number</p>
              <p style="color:white;font-weight:700;font-size:16px;">ORD-${String(id).padStart(4,'0')}</p>
            </div>
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
              ${Utils.getStatusBadge(order.STATUS || order.status)}
              <span style="color:rgba(255,255,255,.8);font-size:11px;">
          ...
          <div class="card-content">
            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:14px;">
              <div><p style="font-size:12px;color:#888;margin-bottom:2px;">Order Type</p>
                <p style="font-size:14px;font-weight:500;">${order.ORDER_TYPE || order.order_type}</p></div>
              <div><p style="font-size:12px;color:#888;margin-bottom:2px;">Payment</p>
                <p style="font-size:14px;font-weight:500;">${order.PAYMENT_METHOD || order.payment_method}</p></div>
            </div>
            ${items.length ? `<div style="border-top:1px solid #f3f4f6;padding-top:12px;margin-bottom:12px;">
              ...
              ${items.map(i => `<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;color:#374151;">
                <span>${i.NAME||i.name} <span style="color:#888;">x${i.QUANTITY||i.quantity}</span></span>
                <span>₱${((i.PRICE||i.price)*(i.QUANTITY||i.quantity)).toFixed(2)}</span></div>`).join('')}
              </div>` : ''}
            <div style="display:flex;justify-content:space-between;align-items:center;
      ...
      container.innerHTML = `<div style="text-align:center;padding:40px;color:#888;">
        ...
        <button onclick="PageInit.orders()" class="btn btn-primary" style="margin-top:16px;">Retry</button></div>`;
    }
  },
  ...
  login: () => {
    if (Auth.isLoggedIn()) {
      ...
      window.location.href = Auth.isAdmin() ? 'admin/dashboard.html' : redirect;
      return;
    }
...
function productCardHTML(p) {
  ...
  return `
    <div class="card product-card">
      ${!available ? `<div class="product-overlay">
        <span class="product-badge badge-unavailable">Unavailable</span></div>` : ''}
      <img src="${p.image || 'https://via.placeholder.com/400'}" alt="${p.name}"
        class="card-image" onerror="this.src='https://via.placeholder.com/400'" />
      <div class="card-content">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn btn-outline btn-sm add-to-cart-btn" style="flex:1;"
          ...
          <button class="btn btn-primary btn-sm buy-now-btn" style="flex:1;"
            data-product="${pJson}" ${!available ? 'disabled' : ''}>
            Buy Now
          </button>
        </div>
...
function setupProductButtons() {
  ...
  document.querySelectorAll('.buy-now-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (!Auth.isLoggedIn()) {
        sessionStorage.setItem('redirectAfterLogin', 'checkout.html');
        // Save the product so checkout knows it's a buy-now after login
        const p = JSON.parse(btn.dataset.product);
...
function updateCartSummary(items) {
  ...
  set('total', `₱${(subtotal + delivery).toFixed(2)}`);
}
