...
// GET /api/orders/notifications/:userId — MUST be before /:id routes
router.get('/notifications/:userId', async (req, res) => {
  ...
  try {
    ...
    // Notifications table might not exist yet — return empty array
    res.json([]);
  } finally { if (conn) await conn.close(); }
});
...
router.put('/:id/status', async (req, res) => {
  ...
  try {
    conn = await getConnection();
    await conn.execute(`UPDATE orders SET status=:status WHERE order_id=:id`, { status, id: parseInt(req.params.id) });
    await conn.commit();
    res.json({ success: true });
    ...
    try {
      await conn.execute(
        ...
        { user_id: userId, order_id: orderId,
          message: `Your order ORD-${String(orderId).padStart(4,'0')} status updated to: ${status}` }
      );
      await conn.commit();
